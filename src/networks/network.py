...
class LLL_Net(nn.Module):
    ...
    def __init__(self, model, remove_existing_head=False, keep_classifier=False, num_classes=10):
        ...
        if keep_classifier:
            try:
                self.heads.append(nn.Linear(self.out_size, self.model.num_classes))
            except:
                print("Classifier output dim: ", num_classes)
                self.heads.append(nn.Linear(self.out_size, num_classes))
    ...
    def add_head(self, num_outputs):
        ...
        nn.init.kaiming_normal_(self.heads[-1].weight, nonlinearity="linear")
        nn.init.constant_(self.heads[-1].bias, 0.0)
