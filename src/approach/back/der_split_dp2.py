...
#             shuffled_exemplars_dataset = deepcopy(self.exemplars_dataset)
        ...
        if t > 0:
            print("Memory logit length before adding new logits to memory: ", len(self.exemplars_dataset.logits))
        ...
        print("exemplars logit: ", type(self.exemplars_dataset.logits))
        print("exemplars images: ", type(self.exemplars_dataset.images))
        print("exemplars labels: ", type(self.exemplars_dataset.labels))
...
#                     outputs_old = self.server_model_old(client_old_outputs)
                if not now_exem:
                    ...
                    images, targets, logits = data[0], data[1], data[2]
...
#                         outputs_old = self.server_model_old(client_old_outputs)
    ...
    def criterion(self, t, outputs, targets, now_exem):
        ...
        if not now_exem:
            ...
            # targets: buffer true logit
#             print("torch.cat(outputs, dim=1): ", torch.cat(outputs[:t], dim=1).shape)
