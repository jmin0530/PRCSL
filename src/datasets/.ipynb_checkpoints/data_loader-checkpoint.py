...
def get_loaders(datasets, num_tasks, nc_first_task, batch_size, num_workers, pin_memory, validation=.1):
    ...
    for idx_dataset, cur_dataset in enumerate(datasets, 0):
        ...
        trn_transform, tst_transform = get_transforms(resize=dc['resize'],
                                                      ...
                                                      crop=dc['crop'],
                                                      flip=dc['flip'],
                                                      vertical_flip=dc['vertical_flip'],
...
def get_transforms(resize, pad, crop, flip, vertical_flip, normalize, rotation, extend_channel):
    ...
    if pad is not None and crop is not None:
        del trn_transform_list[-1]
        del trn_transform_list[-1]
        trn_transform_list.append(transforms.RandomCrop(crop, padding=pad))
    ...
    if rotation is not None:
        trn_transform_list.append(transforms.RandomRotation(30))
